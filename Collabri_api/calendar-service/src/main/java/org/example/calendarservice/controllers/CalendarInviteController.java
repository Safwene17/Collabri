...
public class CalendarInviteController {
    ...
    @PostMapping("/{id}")
    public ResponseEntity<Void> inviteByEmail(@PathVariable("id") UUID calendarId, @RequestParam String destinationEmail, Authentication authentication) {
        inviteService.inviteMember(calendarId, destinationEmail, authentication);
        return ResponseEntity.ok().build();
    }

    @PostMapping("/accept")
    public ResponseEntity<Void> acceptInviteAuthenticated(@RequestBody AcceptInviteRequest req, Authentication authentication) {
        inviteService.acceptInviteWithAuth(req.token(), authentication);
        return ResponseEntity.ok().build();
    }

    @PostMapping("/decline")
    public ResponseEntity<Void> declineInvite(@RequestBody DeclineInviteRequest req, Authentication authentication) {
